...
output_path = os.path.join(output_dir, "phase6plus_color_graph.mp4")
...
trail_speeds = deque(maxlen=trail_length)
...
while True:
    ...
    if results.pose_landmarks:
        ...
        if prev_wrist is not None:
            ...
            trail_speeds.append(smooth_speed_kmh)

            for i in range(1, len(wrist_trail)):
                # Color-coded trail based on speed
                speed = trail_speeds[i]
                if speed < 20:
                    color = (0,255,0)      # Green
                elif speed < 35:
                    ...
                    color = (0,0,255)      # Red
                cv2.line(frame, wrist_trail[i-1], wrist_trail[i], color, 3)
    ...
    if speed_history:
        ...
        max_speed = max(speed_history) * 1.2
        min_speed = 0
        prev_point = None
        for i, speed in enumerate(speed_history):
            ...
            if prev_point is not None:
                cv2.line(graph, prev_point, (x, y), color, 2)
            prev_point = (x, y)
...
print("✅ Phase 6++ video saved at:", output_path)
